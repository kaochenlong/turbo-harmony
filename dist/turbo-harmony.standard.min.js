 ...
 * @license MIT
 */
class e{constructor(e={}){this.options={debug:!1,preserveState:!1,preserveStateSelectors:["[x-data]"],skipSelectors:[".turbo-harmony-skip",".no-alpine","[data-turbo-harmony-skip]"],reinitDelay:0,batchUpdates:!0,beforeReinit:null,afterReinit:null,onError:null,watchAttributes:["x-data","x-show","x-if","x-for"],autoStart:!0,...e},this.isInitialized=!1,this.preservedStates=new WeakMap,this.initializedElements=new WeakSet,this.handleBeforeVisit=this.handleBeforeVisit.bind(this),this.handleLoad=this.handleLoad.bind(this),this.handleBeforeStreamRender=this.handleBeforeStreamRender.bind(this),this.handleStreamRender=this.handleStreamRender.bind(this),this.handleBeforeFrameRender=this.handleBeforeFrameRender.bind(this),this.handleFrameRender=this.handleFrameRender.bind(this),this.handleError=this.handleError.bind(this),this.options.autoStart&&this.init()}init(){return this.isInitialized?(this.log("warn","TurboHarmony already initialized"),this):(this.validateDependencies(),this.setupEventListeners(),this.isInitialized=!0,this.log("info","TurboHarmony initialized successfully",{options:this.options,version:"1.0.0"}),this)}validateDependencies(){if("undefined"==typeof window)throw new Error("TurboHarmony: window object not available");if(!window.Alpine)throw new Error("TurboHarmony: Alpine.js not found. Please ensure Alpine.js is loaded before TurboHarmony.");if(!window.Turbo)throw new Error("TurboHarmony: Turbo not found. Please ensure @hotwired/turbo is loaded before TurboHarmony.");this.log("debug","Dependencies validated successfully")}setupEventListeners(){document.addEventListener("turbo:before-visit",this.handleBeforeVisit),document.addEventListener("turbo:load",this.handleLoad),document.addEventListener("turbo:before-stream-render",this.handleBeforeStreamRender),document.addEventListener("turbo:stream-render",this.handleStreamRender),document.addEventListener("turbo:before-frame-render",this.handleBeforeFrameRender),document.addEventListener("turbo:frame-render",this.handleFrameRender),window.addEventListener("error",this.handleError),window.addEventListener("unhandledrejection",this.handleError),this.log("debug","Event listeners registered")}handleBeforeVisit(e){this.log("debug","Turbo Drive: before visit",{location:e.detail.url}),Alpine.destroyTree&&Alpine.destroyTree(document.body)}handleLoad(e){this.log("debug","Turbo Drive: load complete"),Alpine.initTree&&Alpine.initTree(document.body)}handleBeforeStreamRender(e){this.log("debug","Turbo Stream: before render",{action:e.detail?.action,target:e.detail?.target});const t=this.findTargetElement(e);t&&this.options.preserveState&&this.preserveAlpineState(t)}handleStreamRender(e){try{const t=this.findTargetElement(e);if(!t)return void this.log("warn","Turbo Stream: target element not found",e.detail);if(this.log("debug","Turbo Stream: render detected",{action:e.detail?.action,target:t.tagName,id:t.id,classes:t.className}),this.shouldSkipElement(t))return void this.log("debug","Turbo Stream: skipping element due to skip selector");this.reinitializeAlpineInElement(t)}catch(e){this.handleError(e,"handleStreamRender")}}handleBeforeFrameRender(e){this.log("debug","Turbo Frame: before render",{frame:e.target.id})}handleFrameRender(e){const t=e.target;this.log("debug","Turbo Frame: render complete",{frame:t.id}),this.shouldSkipElement(t)||this.reinitializeAlpineInElement(t)}findTargetElement(e){return e.target||e.detail?.target||e.detail?.selector&&document.querySelector(e.detail.selector)||null}shouldSkipElement(e){return!e||!e.matches||this.options.skipSelectors.some(t=>{try{return e.matches(t)}catch(e){return this.log("warn",`Invalid skip selector: ${t}`,e),!1}})}reinitializeAlpineInElement(e){try{this.log("debug","Reinitializing Alpine in element",{tag:e.tagName,id:e.id,classes:e.className}),this.options.beforeReinit&&this.options.beforeReinit(e),this.options.preserveState&&this.preserveAlpineState(e);const t=e.querySelectorAll("[x-data]"),r=[];if(t.forEach(e=>{e._x_dataStack&&e._x_dataStack.length>0&&this.initializedElements.has(e)?this.log("debug","Skipping already initialized element",{id:e.id,classes:e.className}):r.push(e)}),r.length>0){Alpine.destroyTree&&Alpine.destroyTree(e);const t=()=>{Alpine.initTree?Alpine.initTree(e):Alpine.start&&Alpine.start(),r.forEach(e=>{this.initializedElements.add(e)}),this.options.preserveState&&this.restoreAlpineState(e),this.options.afterReinit&&this.options.afterReinit(e),this.log("debug","Alpine reinitialization complete",{initialized:r.length})};this.options.reinitDelay>0?setTimeout(t,this.options.reinitDelay):t()}else this.log("debug","No elements need reinitialization")}catch(e){this.handleError(e,"reinitializeAlpineInElement")}}preserveAlpineState(e){try{const t=e.querySelectorAll(this.options.preserveStateSelectors.join(",")),r=new Map;t.forEach(e=>{if(e._x_dataStack&&e._x_dataStack.length>0)try{const t=this.safeStringify(e._x_dataStack[0]);r.set(this.getElementKey(e),JSON.parse(t))}catch(e){this.log("warn","Failed to preserve state for element",e)}}),this.preservedStates.set(e,r),this.log("debug",`Preserved state for ${r.size} Alpine components`)}catch(e){this.handleError(e,"preserveAlpineState")}}restoreAlpineState(e){try{const t=this.preservedStates.get(e);if(!t)return;setTimeout(()=>{e.querySelectorAll(this.options.preserveStateSelectors.join(",")).forEach(e=>{const r=this.getElementKey(e),i=t.get(r);if(i&&e._x_dataStack&&e._x_dataStack.length>0)try{const t=e._x_dataStack[0];Object.keys(i).forEach(e=>{const r=Object.getOwnPropertyDescriptor(t,e);if(!r||r.set||!1!==r.writable)try{t[e]=i[e]}catch(t){this.log("debug",`Skipped non-writable property: ${e}`)}}),this.log("debug","Restored state for element",r)}catch(e){this.log("warn","Failed to restore state for element",e)}}),this.preservedStates.delete(e)},10)}catch(e){this.handleError(e,"restoreAlpineState")}}getElementKey(e){return e.id||e.getAttribute("data-key")||`${e.tagName}-${Array.from(e.parentNode.children).indexOf(e)}`}safeStringify(e){const t=new WeakSet;return JSON.stringify(e,(e,r)=>{if(!(r instanceof HTMLElement||r instanceof Window||r instanceof Document||r instanceof Event||"function"==typeof r||"string"==typeof e&&e.startsWith("_"))){if("object"==typeof r&&null!==r){if(t.has(r))return;t.add(r)}return r}})}log(e,t,r=null){this.options.debug&&("error"!==e&&"warn"!==e||console[e]("[TurboHarmony]",t,r||""))}handleError(e,t=""){const r={message:e.message||e,context:t,timestamp:(new Date).toISOString()};if(this.log("error",`TurboHarmony error in ${t}:`,r),this.options.onError)try{this.options.onError(e,r)}catch(e){console.error("[TurboHarmony] Error in custom error handler:",e)}}reinitializeAll(){this.log("info","Manual reinitialization triggered"),this.reinitializeAlpineInElement(document.body)}destroy(){document.removeEventListener("turbo:before-visit",this.handleBeforeVisit),document.removeEventListener("turbo:load",this.handleLoad),document.removeEventListener("turbo:before-stream-render",this.handleBeforeStreamRender),document.removeEventListener("turbo:stream-render",this.handleStreamRender),document.removeEventListener("turbo:before-frame-render",this.handleBeforeFrameRender),document.removeEventListener("turbo:frame-render",this.handleFrameRender),window.removeEventListener("error",this.handleError),window.removeEventListener("unhandledrejection",this.handleError),this.isInitialized=!1,this.log("info","TurboHarmony destroyed")}}"undefined"!=typeof window&&(window.TurboHarmony=e);export{e as TurboHarmony,e as default};
//# sourceMappingURL=turbo-harmony.standard.min.js.map
